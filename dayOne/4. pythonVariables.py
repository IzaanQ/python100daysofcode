input("What is your name? ")

# Reminder: Whatever is passed by the user in the console as a result of this function is returned back to the function

name = input("What is your name? ")

# This is called variable assignment. By doing this, we can assign the input function return value to the variable name. We can refer to
# this value at any time as it is saved in the program.

name = input("What is your name? ")
print(name)

# Variables can be changed in python. It can be re-assigned later in the code or 'updated'

name = "Izaan"
print(name)

name = "qaiser"
print(name)

# There is no protection to keep a variable constant. If it is subject to change, it allows that change without conesting.
